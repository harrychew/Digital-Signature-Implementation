...
def run_process():
    ...
    sign= sign(priv_key_b,hashed)
    bundle=msg+b'`'+hashed.digest()
    encrypted_bundle = encrypt(pub_key_a,bundle)
    #Store signature into file
    SignatureFile = open("BobSignatureFile.txt", "wb")
    SignatureFile.write(sign)
    SignatureFile.close()
    #Store message into file
    MessageFile=open("BobEncryptedMessageFile.txt","wb")
...
def run_process2():
	try:
		...
		MessageFile=open("AliceEncryptedMessageFile.txt","r+")
		encrypted_bundle1=MessageFile.read()
		MessageFile.close()
		SignatureFile = open("AliceSignatureFile.txt", "r+")
		sign=SignatureFile.read()
		SignatureFile.close()
		print(encrypted_bundle1)
		decrypted_bundle=decrypt(priv_key_b,encrypted_bundle1)
		...
		verified_msg= verify(pub_key_a,hashed2,sign)
		...
		a1= 'Getting Alice Signature and Encrypted bundle...\n'
		c = 'Signature is \n'
...
#get Bob's private key file
BobPrivateKeyFile=open("BobPrivateKeyFile.der","r+")
private_key_bob=BobPrivateKeyFile.read()
BobPrivateKeyFile.close()
priv_key_b=RSA.importKey(private_key_bob)
print("Done! Bob's private key received!")
tk.Label(master, text="Bob's private key received!").grid(row=2)
...
tk.Label(master, text="Alice's public key received!").grid(row=3)
...
tk.Label(master, text="Send Message to Alice").grid(row=7)
e1 = tk.Entry(master)
...
tk.Label(master, text="Click NEXT to verify message from Alice").grid(row=12)
